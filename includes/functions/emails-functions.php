...
if ( ! function_exists( 'houzez_safe_wp_mail' ) ) {
    function houzez_safe_wp_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
        try {
            return wp_mail( $to, $subject, $message, $headers, $attachments );
        } catch ( \Throwable $e ) {
            error_log( 'Houzez wp_mail error: ' . $e->getMessage() );
            return false;
        }
    }
}
    ...
    function houzez_send_emails_with_reply( $user_email, $subject, $message, $sender_name, $sender_email, $cc_email, $bcc_email ){
        ...
        if( $enable_email_header != 0 ) {
            $email_content = '<div style="text-align: center; background-color: ' . esc_attr($email_head_bg_color) . '; padding: 16px 0;">
                            <img src="' . esc_url($email_head_logo) . '" alt="' . esc_attr(get_bloginfo('name')) . '">
                        </div>';
        }
        ...
        if ( ! empty( $bcc_email ) ) {
            ...
            foreach ( $bcc_emails as $bcc_email ) {
                houzez_safe_wp_mail( trim( $bcc_email ), $subject, $email_messages, $headers );
            }
        }
        ...
        if( is_array( $user_email ) ) {
            ...
            foreach ($user_email as $email) {
                $email_sent = houzez_safe_wp_mail(
                    $email,
                    $subject,
                    $email_messages,
                    $headers
...
if( !function_exists( 'houzez_contact_realtor' ) ) {
    function houzez_contact_realtor() {
        ...
        if($agent_type == 'author_info') {
            ...
            $target_email = get_post_meta($agent_id, 'fave_agency_email', true) /* TODO unmapped fave_ key: fave_agency_email */;
        } else {
            $target_email = get_post_meta($agent_id, 'fave_agent_email', true) /* TODO unmapped fave_ key: fave_agent_email */;
        }
        ...
        if (houzez_safe_wp_mail( $target_email, $email_subject, $email_body, $headers)) {
            echo json_encode( array(
                'success' => true,
...
if( !function_exists( 'houzez_ele_contact_form' ) ) {
    function houzez_ele_contact_form() {
        ...
        $email_sent = houzez_safe_wp_mail( $email_to, $email_subject, $email_body, $headers . $cc_header );

        if ( ! empty( $email_to_bcc ) ) {
            $bcc_emails = explode( ',', $email_to_bcc );
            foreach ( $bcc_emails as $bcc_email ) {
                houzez_safe_wp_mail( trim( $bcc_email ), $email_subject, $email_body, $headers );
            }
        }
