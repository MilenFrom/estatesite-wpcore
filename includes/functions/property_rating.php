...
if ( !function_exists( 'houzez_rating_meta_filter' ) ) {
    ...
    function houzez_rating_meta_filter( $property_id, $rating_count, $save = true, $old_rating_count = 0 )
    {
        ...
        $rating_data = \EstateSite\Core\Property::get( $property_id, 'rating' );
        ...
        \EstateSite\Core\Property::set( $property_id, 'rating', $rating_data );
    ...
    function houzez_rating_approve( $new_status, $old_status, $comment )
    {
        if ( $old_status != $new_status ) {
            if ( $new_status == 'approved' ) {
                ...
                $rating_data = \EstateSite\Core\Property::get( $comment->comment_post_ID, 'rating' );
                $rating_data[ $rating ]++;
                \EstateSite\Core\Property::set( $comment->comment_post_ID, 'rating', $rating_data );
            } else {
                $rating = get_comment_meta( $comment->comment_ID, 'rating', true );
                $rating_data = \EstateSite\Core\Property::get( $comment->comment_post_ID, 'rating' );
